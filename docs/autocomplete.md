...
3. **`items_dictionary_snapshot()`** (`notifier.rs`) — getter над
   `class_cache`: нормализует каждое имя, фильтрует пустые, сортирует,
   дедупит. Нормализация срезает хвостовые суффиксы в скобках, которые
   в items.txt маркируют варианты одной базы:
   - `(Sacred)` / `(Angelic)` / `(Mastercrafted)` — тиры уника;
   - `(N)` с любым числом цифр — счётчик тира weapon/armor (`(1)`…`(4)`)
     или счётчик варианта shrine / riftstone / fragment (`(4)`…`(10)`).

   **Исключение:** если имя оканчивается на `Container (NN)` — не
   трогаем. Там число идентифицирует конкретную руну (`Amn Container
   (11)`, `Ber Container (30)`) и является частью имени, а не маркером
   варианта. `ClassInfo.base_name` сам не трогается — его читают
   matching и `ItemDropEvent`, нормализация живёт только в snapshot.
