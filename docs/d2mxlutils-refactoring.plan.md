...
- Модуль `notifier` (`src-tauri/src/notifier.rs`):
  ...
  - Метод `tick(&mut self)`:
    ...
    - Возвращает список `item-drop` событий, который Tauri-backend эмитит в UI.
...
6. `rust-drop-scanner`: Собрать DropScanner и сканирование предметов. **(выполнено 01.12.2025, 03.12.2025 — сканер использует имена/статы через инъекции, разделены слои сканирования и формирования событий)**
...
**03.12.2025** — рефакторинг `DropScanner` (разделены слои сканирования и формирования событий, интегрированы вызовы `get_item_name` / `get_item_stats`), в `logger.rs` добавлены таймстампы к файловому логу для удобства анализа релизных запусков.