...
> absence is part of why a real regression sat unnoticed: the shipped Rust
> port's `attach_chain` diverged from the `insertCell` algorithm documented
> below (splicing a fresh root instead of walking to a leaf), which is the
> leading suspect for reports of shrine/quest markers occasionally not
> showing up — the engine's own icon placement reads/writes the same
> `pObjects` root we were repeatedly overwriting. Fixed to match this
> doc's algorithm exactly; see `find_leaf_slot`/`attach_chain` in
> `map_marker.rs`.
...
3. Splice into `pObjects` BST as a leaf. **Order is irrelevant — the
   renderer walks the entire tree.** Simplest insertion: walk down `pLess`
   until it's `NULL`, attach there.
...
> **Load-bearing detail.** This walks to a leaf and never touches the root
> slot unless the tree is empty. `pObjects` is the single most contended
> pointer in this structure — the engine's own quest/shrine-icon placement
> also reads and writes it — so an implementation that instead swaps
> `pObjects` itself to point at a freshly-built chain (even one that
> correctly preserves the old tree via its own tail pointer) is racing
> against the engine's concurrent use of that same slot on every rebuild.
> The Rust port initially did exactly that; it's the leading suspect for
> reports of shrine/quest markers occasionally not rendering, and was
> fixed to match this leaf-walk exactly (`find_leaf_slot` in
> `map_marker.rs`).
...
Implemented in `src-tauri/src/map_marker.rs` (`MapMarkerManager`). Follows
