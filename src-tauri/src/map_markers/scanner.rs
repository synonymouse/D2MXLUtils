...
use super::manager::{self, MapMarkerManager, MarkerItem};
...
impl MarkerScanner {
    ...
    pub fn tick(&mut self) {
            ...
            .read_memory::<u32>(self.state.ctx.d2_client + d2client::PLAYER_UNIT)
        {
            ...
            Err(e) => {
                ...
                log_error(&format!("map_marker player read failed: {}", e));
                return;
            }
        };
        if p_player == 0 {
            self.map_marker.invalidate_cells();
        ...
        let positions = match manager::bfs_item_positions(&self.state.ctx, 10) {
            Ok(v) => v,
            Err(e) => {
                clear_recent_bfs_items(self.state.as_ref());
        ...
        for (p_unit, sub_x, sub_y) in positions {
            ...
            match cached_marker_decision(snapshot.get(&unit_id), current_generation) {
                CachedMarkerDecision::Place => {
                    let (cx, cy) = manager::sub_to_cell(sub_x, sub_y);
        ...
        let player_sub = manager::read_player_subtile(&self.state.ctx);
    ...
    pub fn shutdown(&mut self) {
        clear_recent_bfs_items(self.state.as_ref());
        match self.map_marker.clear(&self.state.ctx) {
