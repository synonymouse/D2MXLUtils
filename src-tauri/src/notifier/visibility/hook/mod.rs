...
pub(in crate::notifier) const MASK_BYTES: usize = 8192;
const MASK_INDEX_BITS: u32 = 0xFFFF;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::notifier) enum VisibilityMaskOp {
    SetShow,
    SetHide,
    ClearShow,
    ClearHide,
...
pub(in crate::notifier) fn visibility_mask_ops(
    visibility: Visibility,
) -> &'static [VisibilityMaskOp] {
    match visibility {
...
#[cfg(any(target_os = "windows", target_os = "linux"))]
pub(in crate::notifier) struct LootFilterHook {
...
impl LootFilterHook {
    ...
    fn fresh_inject(&mut self, ctx: &D2Context, found_addr: usize) -> Result<(), String> {
        ...
        // read+execute-only code pages (see `process/linux.rs`), so no
